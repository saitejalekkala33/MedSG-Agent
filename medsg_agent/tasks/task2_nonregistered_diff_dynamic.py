...
from ..utils import BBox, ensure_same_size, to_gray01, phase_correlation_shift, ecc_register
...
def get_diff_bbox_with_registration_dynamic(img_a: np.ndarray, img_b: np.ndarray, use_phasecorr_first: bool = True, edge_band_frac: float = 0.02, morph: int = 3, base_thresh: float = 0.25) -> Tuple[Optional[BBox], float, Dict]:
    ...
    if _ssim is not None:
        with warnings.catch_warnings():
            ...
            _, ssim_map = _ssim(ws_gray, Bg, full=True)
        diff = 1.0 - ssim_map
    else:
        diff = np.abs(ws_gray - Bg)
    h, w = diff.shape
    band = max(2, int(edge_band_frac * min(h, w)))
    diff[:band,:] = 0; diff[-band:,:] = 0; diff[:,:band] = 0; diff[:,-band:] = 0
    cands = _candidate_thresholds(diff, base_thresh)
    bb, conf, det = _choose_best_threshold_no_gt(diff, morph, cands, band=band)
    det.update({"band": band, "warp": W.tolist(), "search_thresholds": cands})
    return bb, conf, det
