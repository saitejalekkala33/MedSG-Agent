...
# from medsg_agent.utils import read_image, BBox, parse_two_image_indices
...
# # ---------- Tool functions (accept kwargs, not a single args object) ----------
...
# def get_tools():
#     """Return the full list of LangChain StructuredTools for the agent."""
#     return [
...
#     n = len(images)
#     if n < 2:
#         return json.dumps({"bbox": None, "confidence": 0.0, "details": {"error": "need >=2 images"}})
...
# # ---------- Gallery router tool (routes to ALL tasks 1/2/3/5/6/7) ----------
...
# ) -> str:
#     """
#     Route a gallery of images to the correct task, based on the question.
#     Supports tasks: registered_diff, nonregistered_diff, multi_view, concept, patch, crossmodal.
#     """
#     n = len(images)
#     if n < 2:
#         return json.dumps({"bbox": None, "confidence": 0.0, "details": {"error": "need >=2 images"}})
...
#     # Task 6: Patch grounding (source + N region images)
#     if task == "patch":
#         src_path = images[0]
#         r_idx_1based = parse_region_index_from_question(question, default=1)
#         p_idx = r_idx_1based  # images[1] is region 1
#         if p_idx >= n:
#             return json.dumps({"bbox": None, "confidence": 0.0, "details": {"error": f"region index {r_idx_1based} out of range", "task": task}})
...
#             alpha=patch_alpha,
#         )
#         det = {**det, "task": task, "selected_indices": [0, p_idx], "selected_paths": [src_path, images[p_idx]]}
#         return _fmt_out(bbox, conf, det)

#     # For other tasks, we need two images selected
#     i, j = parse_two_image_indices(question, n)
#     if i == j:
#         j = (i + 1) % n
...
#         bbox, conf, det = get_subtracted_bbox(A, B, method=method, thresh=thresh, morph=morph)
#         det = {**det, "task": task, "selected_indices": [i, j], "selected_paths": [A_path, B_path]}
#         return _fmt_out(bbox, conf, det)
...
#         if rb is None:
#             # Fallback: largest component bbox on A
#             m = tight_mask(A)
...
#     if task == "concept":
#         rb = parse_ref_bbox_from_question(question)  # optional
...
#     # Unknown -> default to nonregistered diff
#     bbox, conf, det = get_diff_bbox_with_registration_robust(
#         A, B,
#         use_phasecorr_first=use_phasecorr_first,
#         edge_band_frac=edge_band_frac,
...
#         thresh=thresh,
#     )
#     det = {**det, "task": "nonregistered_diff", "selected_indices": [i, j], "selected_paths": [A_path, B_path]}
#     return _fmt_out(bbox, conf, det)
